...
class AppViewModel extends ChangeNotifier {
  ...
  List<String> getAllMagisterStudies() {
    return ["NBIO", "NISD", "NISY", "NIDE", "NCPS", "NSEC", "NMAT", "NISD", "NISD"];
  }
  ...
  List<int> filterCourses(Category category) {
    ...
    switch (category) {
      case Category.compulsory:
        return List.generate(6, (index) => index + (grade + 1) * 6, growable: false);
      case Category.compulsoryOptional:
        return List.generate(4, (index) => index + ((grade + 1) * 2), growable: false);
      case Category.optional:
        return List.generate(5, (index) => 2 * index + (grade + 4), growable: false);
      default:
        return [];
    }
  }
